...
Nowadays, programs are allocated a region of memory and the processor MMU unit handles memory protection and access alongside the kernel. If a program attempts to access a memory location that is not in its defined region, a *segfault* (UNIX signal SIGSEGV) is triggered.
