...
* Accessing an array's members can be done with pointer arithmetic, *assuming that arrays are stored contiguously*. For example:
  ...
  * Be careful with this. Incrementing a pointer outside of the bounds of an array, and then dereferencing it is **undefined behaviour** (program may be unstable, crash, garbage data may be returned etc).
